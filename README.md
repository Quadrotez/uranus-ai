...
После запуска открой [http://localhost:5173](http://localhost:5173). Интерфейс поддерживает реальные client-side URL: `/` — Агент, `/computer` — Computer, `/settings` — Настройки. Прямой вход и обновление страницы на этих адресах должны оставаться в соответствующем разделе. Админка находится в разделе «Настройки». Для первого запуска local-first режим допускает дефолтный `ADMIN_TOKEN=change-me`; перед публикацией задай собственное значение и добавь reverse proxy/TLS. На Linux при нестандартном UID/GID владельца проекта замени `URANUS_UID` и `URANUS_GID` в `.env` значениями `id -u` и `id -g`.
...
`/api/...` — это backend API, а не страницы интерфейса. Например, `GET http://localhost:8000/health` проверяет API, `GET http://localhost:8000/api/docs` открывает Swagger, а `GET /api/providers/{provider_id}/models` запрашивает каталог у конкретного upstream-провайдера. Если последний endpoint отвечает `502`, endpoint Uranus-AI существует, но провайдер не ответил: проверь ключ, Base URL, модель и Proxy в настройках. Это не означает, что страницы `/settings` или `/computer` отсутствуют.
